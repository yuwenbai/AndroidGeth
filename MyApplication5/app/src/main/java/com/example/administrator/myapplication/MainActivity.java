...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RootDir = "/mnt/sdcard/work/mywork";
...
//判断文件夹是否存在，如果不存在就创建，否则不创建
            ...
            Log.d("111", "InitClient111: " + GenesisString);
            nc.setEthereumGenesis(GenesisString);
            //nc.setEthereumGenesis(Geth.testnetGenesis());
            //nc.setEthereumEnabled(true);
            //nc.setEthereumNetworkID(2265);
            nc.setWhisperEnabled(true);
            nc.setEthereumEnabled(true);
            nc.setEthereumNetworkID(2265);
            node = Geth.newNode(RootDir + "/rex2265",nc);
            node.start();
            EthereumClient ec = node.getEthereumClient();
...
//            String hBlock = ec.syncProgress(ctx).getHighestBlock();
    ...
    {
        //        Geth.newAddresses()
//        KeyStore ks = new KeyStore(RootDir +"/.keystoreandroid",Geth.LightScryptN,Geth.LightScryptP);
//        try{
//            Account newAcc = ks.newAccount("Creation password");
//        } catch (Exception e) {
//
//        }
//       tx.setText("account : "+ks.getAccounts().toString());
    }
    void GetAccountInfo()
    {
                KeyStore ks = new KeyStore(RootDir +"/.keystoreandroid",Geth.LightScryptN,Geth.LightScryptP);
...
//            String abi = "[{\"constant\":true,\"inputs\":[],\"name\":\"say\",\"outputs\":[{\"name\":\"\",\"type\":\"string\"}],\"payable\":false,\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"name\":\"_greeting\",\"type\":\"string\"}],\"payable\":false,\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"}]";
    ...
    void TestGetContract()
    {
        ...
        callMsg.setTo(new Address("0x3041d9bf6c00712bb2bf96258af0bac418c49693".toLowerCase()));//This is the contract address
        callMsg.setData(hexStringToByteArray("954ab4b2"));
        byte[] ret = null;
